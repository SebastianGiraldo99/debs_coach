...
import { formatearMoneda, formatearMonedaCorta } from "@/lib/formato"
import { fraseProyeccionDeuda, proyeccionDeuda } from "@/lib/mock/checkins"
import { usuario } from "@/lib/mock/usuario"

function TooltipDeuda({ active, payload, label }: Partial<TooltipContentProps<number, string>>) {
  ...
  return (
    <figure className="m-0">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <LineChart data={proyeccionDeuda} margin={{ top: 8, right: 8, bottom: 0, left: 8 }}>
          ...
          <YAxis
            ...
            axisLine={false}
            tick={{ fill: "var(--color-ink-mute)", fontSize: 12 }}
            tickFormatter={(v) => formatearMonedaCorta(v, usuario.monedaBase)}
          />
          <Tooltip content={<TooltipDeuda />} cursor={{ stroke: "var(--color-line-strong)" }} />
          <Line
            type="monotone"
            dataKey="deuda"
      ...
      <p className="mt-2 max-w-[65ch] text-menor text-ink-soft text-pretty">{fraseProyeccionDeuda}</p>
    </figure>
  )
}
